 ...
 * 3.32 — 2026-07-15: PROGRESSIVE voiceover — read-aloud now starts within seconds instead of after the whole answer synthesizes. Shared lib/rag/board/voiceover.ts plans a SHORT first chunk + larger prefetched chunks, plays them in order through one <audio> element; same Gemini-TTS engine/voice (no new cost, no new credential). The 🔈 button is now a toggle — click again to STOP, which aborts pending chunk synths (you don't pay for audio you don't hear). Route gained a chunk flag (inline data URL, no Blob litter). Wired in both brain-node + research-overlay.
 * 3.33 — 2026-07-19: voiceover SILENCE fix — 3.32's prefetch fired 3 chunk requests at once, but the Make TTS webhook handles ONE call at a time → bursts returned 502 (logs: mixed 200/502); when chunk 0 was a 502 you heard nothing. Now chunks synthesize strictly ONE AT A TIME in order (producer/consumer; a chunk's audio far outlasts its synth time, so playback stays gapless without concurrency) + one retry per chunk for transient 502s.
 * 3.34 — 2026-07-19: voiceover "reads 2 lines then stops" fix — 3.33's producer/consumer used a hand-rolled wake latch with a LOST-WAKEUP race: if a chunk finished synthesizing in the gap between the consumer's ready-check and its sleep, the signal was lost and playback stalled after the first clip (runtime logs were all 200s, proving synth was fine and the stall was client-side). Replaced the latch with a promise CHAIN — synth stays serialized one-at-a-time (keeps the 3.33 no-burst fix) but playback awaits plain promises, which cannot lose a wakeup. Also clears onended/onerror on settle so a stale event from the previous src can't end a clip early.
 * 3.35 — 2026-07-19: voiceover RAMPED chunks — the gap after the first clip was pipeline math: a 220-char opener (~17s of audio) can't cover a 1200-char follow-up's ~35s synth on the single-call webhook, so playback ran dry ("two lines… long gap… the rest"). Chunk sizes now ramp 220→350→500→700→1000→1200: each clip's playback time covers the next chunk's synth time, so the buffer grows instead of collapsing — fast start AND continuous read.
 */
export const BUILD_VERSION = '3.35';
